...
for n in range(remaining_iterations):
    scenario.sample_task = True
    rewards = []
    if args.debug: print('Number of medoids: {}'.format(len(metalearner.medoid_policies)))
    for mix, policy in enumerate(metalearner.medoid_policies):
        ...
        if args.debug: print('Metalearner policy {} loaded'.format(mix))
        _, r = metalearner.train(num_obs, update_counter, save=False)
        if args.debug: print('Number of medoids after train: {}'.format(len(metalearner.medoid_policies)))
        if args.debug: print('Initial trajectory completed for task {} and policy {}'.format(n, mix))
        rewards.append(r)
    max_reward_ix = np.array(rewards).argsort()[-1]
    selected_policy = metalearner.medoid_policies[max_reward_ix]
    if args.debug: print('Best medoid policy identified!')
    metalearner.current_policy = selected_policy['policy']
    metalearner.optimizer = selected_policy['optimizer']
    if args.debug: print('Running medoid policy on new entity...')
    metalearner.train(args.k - num_obs, update_counter, save=True)  # Do remaining updates
    ...
    if (n % args.medoid_size == 0) and (n != 0):
        if args.debug: print('Updating medoids...')
        # Update distances
        metalearner.calculate_distances(iterations=[update_counter])  
        if args.debug: print('Distances calculated!')
        metalearner.update_medoids(update_counter, k=args.num_medoids)
        if args.debug: print('Medoids updated!')
        update_counter += 1

# Now metalearner.medoid_policies should have the medoid policies #

# EVALUATION #
if args.debug: print('Evaluating meta-learner...')
eval_agents = args.eval_agents.split(' ')
...
for agent in eval_agents:
    if args.debug: print('Evaluating on agent: {}'.format(agent))
